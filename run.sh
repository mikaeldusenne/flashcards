...
case "$CMD" in
    ...
    update-frontend)
        ...
        ;;
    build*)
        docker-compose -f frontend.yml -f build-frontend.yml up --abort-on-container-exit $@
        ;;
    lint*)
        docker exec -it mikarezoo-flashcards_vuecli_1 npm run lint
        ;;
    dump-db)
        docker exec -it mikarezoo-flashcards_mongo_docker sh -c "mongodump --username $MONGO_INITDB_ROOT_USERNAME --password $MONGO_INITDB_ROOT_PASSWORD --db thedb --out /dumps/dump_$(date +'%Y-%m-%d_%H-%M-%S')"
        ;;
    restore-db)
    ...
    connect-db)
        ...
        docker exec -it mikarezoo-flashcards_mongo_docker mongo --username $user --password $pass
        ;;
    dev-venv-create)
